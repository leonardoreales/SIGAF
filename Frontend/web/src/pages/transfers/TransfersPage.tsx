...
import {
  ...
  Search, Inbox, ShieldCheck, AlertTriangle,
} from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <TabPanel
        tabs={[
          ...
          { id: 'traslados',   label: 'Traslados',   icon: ArrowLeftRight },
        ]}
        active={tab}
        onChange={setTab}
        variant="pill"
      ...
      {tab === 'solicitudes' && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <KpiWidget variant="mini" icon={Inbox}        label="Total"     value={rStats?.total     ?? 0} color="default" loading={rStatsLoading} />
            <KpiWidget variant="mini" icon={Inbox}        label="Recibidas" value={rStats?.recibida   ?? 0} color="blue"    loading={rStatsLoading} />
            <KpiWidget variant="mini" icon={Clock}        label="Gestión"   value={(rStats?.pendienteGestionActivosFijos ?? 0) + (rStats?.revision ?? 0)} color="amber"   loading={rStatsLoading} />
            <KpiWidget variant="mini" icon={ShieldCheck}  label="Aprobadas" value={rStats?.aprobada   ?? 0} color="emerald" loading={rStatsLoading} />
            <KpiWidget variant="mini" icon={CheckCircle2} label="Cerradas"  value={(rStats?.firmada ?? 0) + (rStats?.respuestaEnviada ?? 0)} color="emerald" loading={rStatsLoading} />
          </div>
          ...
          {rError && (
            <EmptyState
              ...
              desc="Verifica la conexión con el API e intenta de nuevo."
            />
          )}
      ...
      {tab === 'traslados' && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <KpiWidget variant="mini" icon={ArrowLeftRight} label="Total"       value={tStats?.total      ?? 0} color="default" loading={tStatsLoading} />
            <KpiWidget variant="mini" icon={Clock}          label="Pendientes"  value={tStats?.pendiente  ?? 0} color="amber"   loading={tStatsLoading} />
            <KpiWidget variant="mini" icon={CheckCircle2}   label="Completados" value={tStats?.completado ?? 0} color="emerald" loading={tStatsLoading} />
            <KpiWidget variant="mini" icon={XCircle}        label="Cancelados"  value={tStats?.cancelado  ?? 0} color="default" loading={tStatsLoading} />
          </div>
          ...
          {tError && (
            <EmptyState
              icon={AlertTriangle}
              title="Error al cargar traslados"
              desc="Verifica la conexión con el API e intenta de nuevo."
            />
          )}
